...
#[starknet::interface]
trait ISwapCallee<T> {
    fn swap_call(ref self: T, sender: ContractAddress, amount0Out: u256, amount1Out: u256, data: Array::<felt252>
    );
}
...
mod Pair {
    ...
    use super::{
        IERC20Dispatcher, IERC20DispatcherTrait, IFactoryDispatcher, IFactoryDispatcherTrait, ISwapCalleeDispatcher, ISwapCalleeDispatcherTrait
    };
    ...
    fn constructor(ref self: ContractState, token0: ContractAddress, token1: ContractAddress) {
        ...
        ERC20::InternalImpl::initializer(ref erc20_state, 'xyzDEX Pair', 'xyz-P');
    ...
    #[external(v0)]
    impl Pair of super::IPair<ContractState> {
        ...
        fn mint(ref self: ContractState, to: ContractAddress) -> u256 {
            ...
            let _total_supply = Pair::total_supply(@self);
            ...
            self.emit(Mint {sender: get_caller_address(), amount0: amount0, amount1: amount1}); // TODO?? sender address instead of caller address
        ...
        fn burn(ref self: ContractState, to: ContractAddress) -> (u256, u256) {
            ...
            let liquidity = Pair::balance_of(@self, self_address);
            let fee_on = InternalImpl::_mint_protocol_fee(ref self, reserve0, reserve1);
            let _total_supply = Pair::total_supply(@self);
            ...
            self.emit(Burn {sender: get_caller_address(), amount0: amount0, amount1: amount1, to: to});
        ...
        fn swap(ref self: ContractState, amount0Out: u256, amount1Out: u256, to: ContractAddress, data: Array::<felt252>) {
            InternalImpl::_check_and_lock(ref self);
            ...
            if (data.len() > 0) {
                let SwapCalleeDispatcher = ISwapCalleeDispatcher { contract_address: to };
                SwapCalleeDispatcher
                    .swap_call(get_caller_address(), amount0Out, amount1Out, data);
            }
            ...
            self.emit(Swap {sender: get_caller_address(), amount0In: amount0In, amount1In: amount1In, amount0Out: amount0Out, amount1Out: amount1Out, to: to});
    ...
    impl InternalImpl of InternalTrait {
        ...
        fn _mint_protocol_fee(ref self: ContractState, reserve0: u256, reserve1: u256) -> bool {
            ...
            if (fee_on) {
                if (klast != 0.into()) {
                    ...
                    if (rootk > rootklast) {
                        let numerator = Pair::total_supply(@self) * (rootk - rootklast);
        ...
        fn _update(ref self: ContractState, balance0: u256, balance1: u256, reserve0: u256, reserve1: u256) {
